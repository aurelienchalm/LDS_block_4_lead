...
def trigger_jenkins_predict_test(**context):
    ...
    # 2. Déclencher le job Jenkins
    build_resp = requests.post(
        f"{conn.host}/job/test_predict/build",
        auth=(username, password),
        headers=headers
    )
    ...
    for _ in range(60):
        build_info = requests.get(
            f"{conn.host}/job/test_predict/{build_number}/api/json",
            auth=(username, password)
        ).json()

        if not build_info["building"]:
            result = build_info["result"]
            jenkins_url = f"{conn.host}/job/test_predict/{build_number}/"
...
with DAG(
    dag_id="housing_predict_test_and_set_tag",
    default_args=default_args,
    start_date=datetime(2025, 8, 4),
    schedule_interval=None,
    catchup=False,
    tags=["jenkins", "predict", "tests"],
    on_failure_callback=send_failure_email,
) as dag:

    trigger_predict = PythonOperator(
        task_id="trigger_jenkins_predict_test_and_set_tag",
        python_callable=trigger_jenkins_predict_test,
    )