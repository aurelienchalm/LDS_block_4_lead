...
Application de prediction de prix de biens immobiliers à partir d'un dataset d'entrainnement.
...
Cette insertion en BDD est faite à l'initialisation du projet par src/load_to_db.py, on peut être amené à ré-utiliser ce script pour insérer de la nouvelle donnée pour executer un nouvel entrainnement du modèle. Concrétement à partir d'un nouveau dataset avec des biens vendu récement avec les prix du marché.Ou si présence d'un data drift remonté par Evidently.

En local : python load_to_db.py
Via airflow le dag dag_load_to_db.py permet de charger la donnée en DB 
